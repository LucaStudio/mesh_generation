...
@function
def transfiniteinterpolation3D(Nx=None,Ny=None,Nz=None,compdomain=None,xi_min=None,xi_max=None,eta_min=None,eta_max=None,zeta_min=None,zeta_max=None,interpolanttype=None,f1=None,f2=None,f3=None,f4=None,f5=None,f6=None,e1=None,e2=None,e3=None,e4=None,e5=None,e6=None,e7=None,e8=None,e9=None,e10=None,e11=None,e12=None,c1=None,c2=None,c3=None,c4=None,c5=None,c6=None,c7=None,c8=None,*args,**kwargs):
    varargin = transfiniteinterpolation3D.varargin
    nargin = transfiniteinterpolation3D.nargin
...
#                 2 --> Hermite
    ...
    N=dot(dot(Nx,Ny),Nz)
    mesh=zeros(N,3)
    if 1 == interpolanttype:
        for s in arange(1,Nz).reshape(-1):
            for r in arange(1,Ny).reshape(-1):
                for p in arange(1,Nx).reshape(-1):
                    xi=compdomain[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),1]
                    eta=compdomain[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),2]
                    zeta=compdomain[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),3]
                    mesh[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),1]=Lagrangeinterps1D(cat([xi_min],[xi_max]),cat([f5[r + dot((s - 1),Ny),1]],[f3[r + dot((s - 1),Ny),1]]),xi) + Lagrangeinterps1D(cat([eta_min],[eta_max]),cat([f2[p + dot((s - 1),Nx),1]],[f4[p + dot((s - 1),Nx),1]]),eta) + Lagrangeinterps1D(cat([zeta_min],[zeta_max]),cat([f1[p + dot((r - 1),Nx),1]],[f6[p + dot((r - 1),Nx),1]]),zeta) - Lagrangeinterps2D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([e5[s,1],e8[s,1]],[e6[s,1],e7[s,1]]),xi,eta) - Lagrangeinterps2D(cat([xi_min],[xi_max]),cat([zeta_min],[zeta_max]),cat([e4[r,1],e12[r,1]],[e2[r,1],e10[r,1]]),xi,zeta) - Lagrangeinterps2D(cat([eta_min],[eta_max]),cat([zeta_min],[zeta_max]),cat([e1[p,1],e9[p,1]],[e3[p,1],e11[p,1]]),eta,zeta) + Lagrangeinterps3D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([zeta_min],[zeta_max]),cat(3,cat([c1[1,1],c4[1,1]],[c5[1,1],c8[1,1]]),cat([c2[1,1],c3[1,1]],[c6[1,1],c7[1,1]])),xi,eta,zeta)
                    mesh[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),2]=Lagrangeinterps1D(cat([xi_min],[xi_max]),cat([f5[r + dot((s - 1),Ny),2]],[f3[r + dot((s - 1),Ny),2]]),xi) + Lagrangeinterps1D(cat([eta_min],[eta_max]),cat([f2[p + dot((s - 1),Nx),2]],[f4[p + dot((s - 1),Nx),2]]),eta) + Lagrangeinterps1D(cat([zeta_min],[zeta_max]),cat([f1[p + dot((r - 1),Nx),2]],[f6[p + dot((r - 1),Nx),2]]),zeta) - Lagrangeinterps2D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([e5[s,2],e8[s,2]],[e6[s,2],e7[s,2]]),xi,eta) - Lagrangeinterps2D(cat([xi_min],[xi_max]),cat([zeta_min],[zeta_max]),cat([e4[r,2],e12[r,2]],[e2[r,2],e10[r,2]]),xi,zeta) - Lagrangeinterps2D(cat([eta_min],[eta_max]),cat([zeta_min],[zeta_max]),cat([e1[p,2],e9[p,2]],[e3[p,2],e11[p,2]]),eta,zeta) + Lagrangeinterps3D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([zeta_min],[zeta_max]),cat(3,cat([c1[1,2],c4[1,2]],[c5[1,2],c8[1,2]]),cat([c2[1,2],c3[1,2]],[c6[1,2],c7[1,2]])),xi,eta,zeta)
                    mesh[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),3]=Lagrangeinterps1D(cat([xi_min],[xi_max]),cat([f5[r + dot((s - 1),Ny),3]],[f3[r + dot((s - 1),Ny),3]]),xi) + Lagrangeinterps1D(cat([eta_min],[eta_max]),cat([f2[p + dot((s - 1),Nx),3]],[f4[p + dot((s - 1),Nx),3]]),eta) + Lagrangeinterps1D(cat([zeta_min],[zeta_max]),cat([f1[p + dot((r - 1),Nx),3]],[f6[p + dot((r - 1),Nx),3]]),zeta) - Lagrangeinterps2D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([e5[s,3],e8[s,3]],[e6[s,3],e7[s,3]]),xi,eta) - Lagrangeinterps2D(cat([xi_min],[xi_max]),cat([zeta_min],[zeta_max]),cat([e4[r,3],e12[r,3]],[e2[r,3],e10[r,3]]),xi,zeta) - Lagrangeinterps2D(cat([eta_min],[eta_max]),cat([zeta_min],[zeta_max]),cat([e1[p,3],e9[p,3]],[e3[p,3],e11[p,3]]),eta,zeta) + Lagrangeinterps3D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([zeta_min],[zeta_max]),cat(3,cat([c1[1,3],c4[1,3]],[c5[1,3],c8[1,3]]),cat([c2[1,3],c3[1,3]],[c6[1,3],c7[1,3]])),xi,eta,zeta)
    else:
        if 2 == interpolanttype:
            for s in arange(1,Nz).reshape(-1):
                for r in arange(1,Ny).reshape(-1):
                    for p in arange(1,Nx).reshape(-1):
                        xi=compdomain[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),1]
                        eta=compdomain[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),2]
                        zeta=compdomain[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),3]
                        mesh[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),1]=Hermiteinterps1D(cat([xi_min],[xi_max]),cat([e2[r,1]],[e4[r,1]]),cat([e2[r,3]],[e4[r,3]]),xi) + Hermiteinterps1D(cat([eta_min],[eta_max]),cat([e1[p,1]],[e3[p,1]]),cat([e2[r,4]],[e4[r,4]]),eta) - Hermiteinterps2D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([c1[1,1],c4[1,1]],[c2[1,1],c3[1,1]]),cat([c1[1,3],c4[1,3]],[c2[1,3],c3[1,3]]),cat([c1[1,4],c4[1,4]],[c2[1,4],c3[1,4]]),cat([c1[1,7],c4[1,7]],[c2[1,7],c3[1,7]]),xi,eta)
                        mesh[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),2]=Hermiteinterps1D(cat([xi_min],[xi_max]),cat([e2[r,2]],[e4[r,2]]),cat([e2[r,5]],[e4[r,5]]),xi) + Hermiteinterps1D(cat([eta_min],[eta_max]),cat([e1[p,2]],[e3[p,2]]),cat([e2[r,6]],[e4[r,6]]),eta) - Hermiteinterps2D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([c1[1,2],c4[1,2]],[c2[1,2],c3[1,2]]),cat([c1[1,5],c4[1,5]],[c2[1,5],c3[1,5]]),cat([c1[1,6],c4[1,6]],[c2[1,6],c3[1,6]]),cat([c1[1,8],c4[1,8]],[c2[1,8],c3[1,8]]),xi,eta)
                        mesh[p + dot((r - 1),Nx) + dot(dot((s - 1),Nx),Ny),3]=Hermiteinterps1D(cat([xi_min],[xi_max]),cat([e2[r,2]],[e4[r,2]]),cat([e2[r,5]],[e4[r,5]]),xi) + Hermiteinterps1D(cat([eta_min],[eta_max]),cat([e1[p,2]],[e3[p,2]]),cat([e2[r,6]],[e4[r,6]]),eta) - Hermiteinterps2D(cat([xi_min],[xi_max]),cat([eta_min],[eta_max]),cat([c1[1,2],c4[1,2]],[c2[1,2],c3[1,2]]),cat([c1[1,5],c4[1,5]],[c2[1,5],c3[1,5]]),cat([c1[1,6],c4[1,6]],[c2[1,6],c3[1,6]]),cat([c1[1,8],c4[1,8]],[c2[1,8],c3[1,8]]),xi,eta)
    ...
    f1=f1[Nx + 1:end() - Nx,:]
    f1temp=zeros(dot((Nx - 2),(Ny - 2)),3)
    for i in arange(1,Ny - 2).reshape(-1):
        f1temp[dot((i - 1),(Nx - 2)) + 1:dot(i,(Nx - 2)),1:3]=f1[dot((i - 1),Nx) + 2:dot(i,Nx) - 1,1:3]
    
    f1=copy(f1temp)
    clear('f1temp')
    mesh[indicesF1,:]=f1[:,:]
    f6=f6[Nx + 1:end() - Nx,:]
    f6temp=zeros(dot((Nx - 2),(Ny - 2)),3)
    for i in arange(1,Ny - 2).reshape(-1):
        f6temp[dot((i - 1),(Nx - 2)) + 1:dot(i,(Nx - 2)),1:3]=f6[dot((i - 1),Nx) + 2:dot(i,Nx) - 1,1:3]
    
    f6=copy(f6temp)
    clear('f6temp')
    mesh[indicesF6,:]=f6[:,:]
    f2=f2[Nx + 1:end() - Nx,:]
    f2temp=zeros(dot((Nx - 2),(Nz - 2)),3)
    for i in arange(1,Nz - 2).reshape(-1):
        f2temp[dot((i - 1),(Nx - 2)) + 1:dot(i,(Nx - 2)),1:3]=f2[dot((i - 1),Nx) + 2:dot(i,Nx) - 1,1:3]
    
    f2=copy(f2temp)
    clear('f2temp')
    mesh[indicesF2,:]=f2[:,:]
    f4=f4[Nx + 1:end() - Nx,:]
    f4temp=zeros(dot((Nx - 2),(Nz - 2)),3)
    for i in arange(1,Nz - 2).reshape(-1):
        f4temp[dot((i - 1),(Nx - 2)) + 1:dot(i,(Nx - 2)),1:3]=f4[dot((i - 1),Nx) + 2:dot(i,Nx) - 1,1:3]
    
    f4=copy(f4temp)
    clear('f4temp')
    mesh[indicesF4,:]=f4[:,:]
    f3=f3[Ny + 1:end() - Ny,:]
    f3temp=zeros(dot((Ny - 2),(Nz - 2)),3)
    for i in arange(1,Nz - 2).reshape(-1):
        f3temp[dot((i - 1),(Ny - 2)) + 1:dot(i,(Ny - 2)),1:3]=f3[dot((i - 1),Ny) + 2:dot(i,Ny) - 1,1:3]
    
    f3=copy(f3temp)
    clear('f3temp')
    mesh[indicesF3,:]=f3[:,:]
    f5=f5[Ny + 1:end() - Ny,:]
    f5temp=zeros(dot((Ny - 2),(Nz - 2)),3)
    for i in arange(1,Nz - 2).reshape(-1):
        f5temp[dot((i - 1),(Ny - 2)) + 1:dot(i,(Ny - 2)),1:3]=f5[dot((i - 1),Ny) + 2:dot(i,Ny) - 1,1:3]
    ...
    lattice=matlabarray(cat(compdomain,mesh,mesh))
    return lattice