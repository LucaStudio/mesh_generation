...
###########################################################################
    ...
    f1=figure()
    for i in arange(1,dot(Nx,Ny)).reshape(-1):
    ...
    lattice[point2,5:6]=lattice[point1,5:6] + cat(dot(dot(rand(),0.5),deltax),dot(dot(rand(),0.5),deltay))
    lattice[point1,5:6]=temp + cat(dot(dot(rand(),0.5),deltax),dot(dot(rand(),0.5),deltay))
    scrsz=get(0,'ScreenSize')
    f3=figure()
    for i in arange(1,dot(Nx,Ny)).reshape(-1):
        if i != point1 and i != point2:
    ...
    metriccoefficients,g,sqrtg=computemetriccoefficients2D(covariantbase,nargout=3)
    contravariantbase=computecontravariantbase2D(covariantbase,sqrtg)
    reciprocalmetriccoefficients,g,sqrtg=computereciprocalmetriccoefficients2D(contravariantbase,nargout=3)
