...
###########################################################################
    ...
    x=- c
    c1[1,1]=x0 - c
    c1[1,2]=y0 - multiply(b,sqrt(1 - (x / a) ** 2))
    x=copy(c)
    c2[1,1]=x0 + c
    c2[1,2]=y0 - multiply(b,sqrt(1 - (x / a) ** 2))
    x=copy(c)
    c3[1,1]=x0 + c
    c3[1,2]=y0 + multiply(b,sqrt(1 - (x / a) ** 2))
    x=- c
    c4[1,1]=x0 - c
    c4[1,2]=y0 + multiply(b,sqrt(1 - (x / a) ** 2))
    x=(arange(- c,c,dot(2,c) / (Neta1 - 1))).T
    e1[:,1]=x0 + x
    e1[:,2]=y0 - multiply(b,sqrt(1 - (x / a) ** 2))
    delta=(a - c) / (Neta2 / 2 - 1)
    x1=(arange(c,a - delta,(a - delta - c) / (Neta2 / 2 - 1))).T
    x2=(arange(a - delta,c,- (a - delta - c) / (Neta2 / 2 - 1))).T
    x=matlabarray(cat([x1],[x2]))
    e4[:,1]=x0 + x
    e4[:,2]=y0 + cat([multiply(- b,sqrt(1 - (x1 / a) ** 2))],[multiply(b,sqrt(1 - (x2 / a) ** 2))])
    x=(arange(- c,c,dot(2,c) / (Neta1 - 1))).T
    e3[:,1]=x0 + x
    ...
    x=matlabarray(cat([x1],[x2]))
    e2[:,1]=x0 + x
    e2[:,2]=y0 + cat([multiply(- b,sqrt(1 - (x1 / a) ** 2))],[multiply(b,sqrt(1 - (x2 / a) ** 2))])
