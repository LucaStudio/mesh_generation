...
###########################################################################
    ...
    c3=zeros(1,8)
    c4=zeros(1,8)
    c1[1,1:2]=cat(x2,y2 - R2)
    c2[1,1:2]=cat(x1,y1 - R1)
    c3[1,1:2]=cat(x1,y1 + R1)
    c4[1,1:2]=cat(x2,y2 + R2)
    xs=(arange(c1[1],c2[1],(c2[1] - c1[1]) / (Neta1 - 1))).T
    me1=(c2[2] - c1[2]) / (c2[1] - c1[1])
    e1[:,1]=xs
    e1[:,2]=multiply(me1,(xs - c1[1])) + c1[2]
    theta=(arange(dot(- 0.5,pi),dot(0.5,pi),pi / (Neta2 - 1))).T
    e4[:,1]=x1 + dot(R1,cos(theta))
    e4[:,2]=y1 + dot(R1,sin(theta))
    xs=(arange(c4[1],c3[1],(c3[1] - c4[1]) / (Neta1 - 1))).T
    me3=(c3[2] - c4[2]) / (c3[1] - c4[1])
    e3[:,1]=xs
    e3[:,2]=multiply(me3,(xs - c4[1])) + c4[2]
